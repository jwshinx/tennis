...
class NbaController < ApplicationController
  ...
  def index
    @performances = Performance.find(:all, :select => 'id, name, pts, rb, game')
  ...
  def cspts
    @csptstime = Benchmark.measure do
      @performances = Performance.find(:all)
      @sorttime = Benchmark.measure do
        @sortedperformances = countsort(@performances)
      end
    end
    @sortedpts = @sortedperformances.map(&:pts)
    respond_to do |format|
      format.html
      format.xml 
    end
  ...
  def qspts
    @qsptstime = Benchmark.measure do
      @performances = Performance.find(:all)
      @sorttime = Benchmark.measure do
        @sortedperformances = quicksort(@performances)
      end
    end
    @sortedpts = @sortedperformances.map(&:pts)
    respond_to do |format|
  ...
  def mspts
    @msptstime = Benchmark.measure do
      @performances = Performance.find(:all)
      @sorttime = Benchmark.measure do
        @sortedperformances = mergesort_objects(@performances)
      end # @sorttime
    end # @msptstime
    @sortedpts = @sortedperformances.map(&:pts)
    @sortedpts
  ...
  def top_scorers
    ...
    @top_passers = Performance.find(:all, :order => "ass DESC", :limit => 5)
